...
- **Markdown in and out** — `create_document_from_markdown` builds a full document in one call, optionally on a brand template via `template=` (styles, headers/footers and page setup inherited; [ADR 0006](docs/adr/0006-template-based-creation.md)); `get_document_markdown` reads structure back for verification, with `{++ins++}`/`{--del--}` revision markup; `set_update_fields_on_open` keeps TOC/page fields fresh ([ADR 0004](docs/adr/0004-markdown-verification-and-authoring.md)).
